...
from app.modules.enrollment.infrastructure.models import Grado, Acudiente, Estudiante, Periodo

def seed_grados():
    SQLModel.metadata.create_all(engine)
    with Session(engine) as session:
        # 1. Crear Acudiente genérico si no existe
        acudiente = session.exec(select(Acudiente).where(Acudiente.nombre == "Acudiente Prueba")).first()
        if not acudiente:
            acudiente = Acudiente(
        ...
        for i in range(1, 6):
            doc = f"100000010{i}"
            est = session.exec(select(Estudiante).where(Estudiante.documento == doc)).first()
            if not est:
        ...
        p1 = session.exec(select(Periodo).where(Periodo.periodo_electivo == datetime(2026, 1, 1))).first()
        if not p1:
            session.add(Periodo(
        ...
        p2 = session.exec(select(Periodo).where(Periodo.periodo_electivo == datetime(2026, 7, 1))).first()
        if not p2:
            session.add(Periodo(
        ...
        session.commit()
        print("[OK] Grados 10 y 11 con estudiantes, y periodos 2026 creados exitosamente.")
