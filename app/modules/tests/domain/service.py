...
class InternalTestService:
    ...
    async def delete_test_complementary(self, comp_id: int) -> bool:
        comp = await self.repository.get_complementary_by_id(comp_id)
        if not comp:
            raise ValueError("Prueba no encontrada")

        return await self.repository.delete_test_complementary(comp_id)
    ...
    async def update_test_complementary(
        ...
        saved_comp = await self.repository.save_complementary(comp)
        return int(saved_comp.id) if saved_comp.id is not None else 0
    ...
    async def register_test_payment(self, test_id: int, request: PaymentRequest):
        ...
        comp = await self.repository.get_complementary_by_id(test.complementario_id)
        if not comp:
