...
from app.modules.classroom_holder.schemas.response import IncidenciaResponse, PazYSalvoClassroomResponse
...
from app.modules.classroom_holder.application.create_incident import CreateIncidentUseCase
...
@router.get("/paz-y-salvo/verificar/{estudiante_id}", response_model=PazYSalvoClassroomResponse)
def verificar_paz_y_salvo(estudiante_id: int, session: SessionDep):
    ...
    mensaje = (
        ...
        if cumple else
        "Paz y salvo denegado: El estudiante presenta reportes abiertos en el Observador."
    )
