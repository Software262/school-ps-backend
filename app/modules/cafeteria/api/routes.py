...
def get_student_service(session: SessionDep) -> StudentService:
...
async def get_debtors_list(
    ...
    use_case = GetStatus(session, student_service)
...
async def search_students(
    ...
    use_case = SearchGeneralStudents(session, student_service)
...
async def get_grades(
    session: SessionDep, student_service: StudentService = Depends(get_student_service)
):
    use_case = GetGrades(session, student_service)
...
async def add_debt(
    ...
    use_case = CreateObservation(session, student_service)
    try:
        data = await use_case.execute(
...
async def clear_debts(
    session: SessionDep,
    request: BulkRemoveBlockRequest,
    student_service: StudentService = Depends(get_student_service),
):
    use_case = UpdateStatus(session, student_service)
...
async def get_individual_status(
    ...
    use_case = GetStudentStatus(session, student_service)
...
async def export_csv(
    ...
    use_case = ExportReport(session, student_service)
